...
namespace apsi {
    void PSIParams::initialize()
    {
        ...
        for (uint32_t p : query_params_.query_powers) {
            ...
            if ((p > query_params_.ps_low_degree) && (p % ps_high_degree) != 0) {
                throw invalid_argument("query_powers cannot contain values larger than ps_low_degree that are not "
        ...
        if (!seal_context.key_context_data()->qualifiers().using_batching) {
            throw invalid_argument("Microsoft SEAL parameters do not support batching; plain_modulus must be a prime "
                                   "congruent to 1 modulo 2*poly_modulus_degree");
        }

        // Compute the bit-length of an item
        item_bit_count_per_felt_ = static_cast<uint32_t>(seal_params_.plain_modulus().bit_count() - 1);
        ...
        items_per_bundle_ = static_cast<uint32_t>(seal_params_.poly_modulus_degree()) / item_params_.felts_per_item;
        ...
        // Compute bins_per_bundle k times felts_per_item
        bins_per_bundle_ = items_per_bundle_ * item_params_.felts_per_item; // namely a slot is a bundle

        // table_size must be a multiple of items_per_bundle_
        if (table_params_.table_size % items_per_bundle_) {
            throw invalid_argument("table_size must be a multiple of floor(poly_modulus_degree / felts_per_item)");
        }
    ...
    size_t PSIParams::save(ostream &out) const
    {
        ...
        fbs::TableParams table_params(
            table_params_.table_size, table_params_.max_items_per_bin, table_params_.hash_func_count);

        // There may or may not be query powers
        vector<uint32_t> query_powers_vec;
        copy(query_params_.query_powers.cbegin(), query_params_.query_powers.cend(), back_inserter(query_powers_vec));
        auto query_powers = fbs_builder.CreateVector(query_powers_vec);
        auto query_params = fbs::CreateQueryParams(fbs_builder, query_params_.ps_low_degree, query_powers);

        vector<seal_byte> temp;
        temp.resize(safe_cast<size_t>(seal_params_.save_size(compr_mode_type::none)));
        size_t size = static_cast<size_t>(seal_params_.save(temp.data(), temp.size(), compr_mode_type::none));
        auto seal_params_data = fbs_builder.CreateVector(reinterpret_cast<const uint8_t *>(temp.data()), size);
    ...
    pair<PSIParams, size_t> PSIParams::Load(istream &in)
    {
        ...
        if (!same_serialization_version(psi_params->version())) {
            ...
            APSI_LOG_ERROR(
                ...
                << psi_params->version() << ") incompatible with the current serialization version number ("
    ...
    PSIParams PSIParams::Load(const string &in)
    {
        ...
        try {
            ...
            if (json_seal_params.isMember("plain_modulus") && json_seal_params.isMember("plain_modulus_bits")) {
                throw runtime_error("only one of plain_modulus and plain_modulus_bits must be specified");
            }
    ...
    string PSIParams::to_string() const
    {
        ...
        ss << "item_params.felts_per_item: " << item_params_.felts_per_item
           ...
           << util::to_string(
                  seal_params_.coeff_modulus(), [](const Modulus &mod) { return std::to_string(mod.bit_count()); })
