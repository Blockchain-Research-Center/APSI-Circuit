...
{ // Constant-time table lookup to extract a point represented as (X+Y,Y-X,2Z,2dT) corresponding to
  ...
  // sign_mask=0
    ...
    temp_point[0] =
        _mm256_loadu_si256((__m256i *)point + 1); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    temp_point[0] = _mm256_loadu_pd(
        (double const *)point + 4); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    point[0] =
        _mm256_blendv_pd(temp_point[0], point[0], full_mask); // If sign_mask = 0 then choose negative of the point
...
{ // Constant-time table lookup to extract a point represented as (x+y,y-x,2t) corresponding to
  ...
  // (sign=-1) then P = -table[digit]
    ...
    temp_point[2] = _mm256_loadu_pd(
        (double const *)point + 2 * 4); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    point[0] =
        _mm256_blendv_pd(point[0], temp_point[0], full_mask); // If sign = 0xFF...F then choose negative of the point
    point[1] = _mm256_blendv_pd(point[1], temp_point[1], full_mask);
    point[2] = _mm256_blendv_pd(point[2], temp_point[2], full_mask);
    _mm256_storeu_pd((double *)P->xy, point[0]);
    _mm256_storeu_pd((double *)P->yx, point[1]);
